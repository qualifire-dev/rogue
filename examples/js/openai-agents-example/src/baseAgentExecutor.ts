...
export abstract class BaseAgentExecutor implements AgentExecutor {
  ...
  async execute(
    ...
    const existingTask: Task | undefined = requestContext.task;

    // Determine IDs for the task and context
    const taskId: string = existingTask?.id || uuidv4();
    const contextId: string = userMessage.contextId || existingTask?.contextId || uuidv4();
